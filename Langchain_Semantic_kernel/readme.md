...
|:----:|:-----------------------------------------------:|:----------------------------------------------------------:|
| 장점 |- 기능을 붙이는 속도가 빠르다                      |- 오픈소스를 잘 관리한다 (깃헙 프로젝트 / PR)                  |
|      |- ecosystem이 풍부하다                            |- LLM뿐 아니라 AI 모델 자체를 <br> 서비스에 통합하는 것에 초점  |
|      |- 필요한 것 대부분 구현되어 있다                    |- 확장성이 좋음                                              |
|      |- 많은 연구 & 기업이 활용해 자료 많다               |- 의존성이 굉장히 가볍다                                      |
|      |- 최신 연구 코드를 쉽게 얻을 수 있다                |- MS의 Copilot 전략 핵심 (오케스트레이션)                     |
|      |- Production Case Study 만들기 위해 지원해준다      |- 지속적 발전이 기대됨                                       |
| 단점 |- 오픈소스 관리 측면에서 아쉬움 <br> (너무 많은 Issue, PR) |- PR 올려도 Merge까지는 오래 걸림 <br> (리뷰 꼼꼼히 해서) |
|      |- 패치가 너무 많다 (지속적으로 버전 맞춰야)           |- 개발 속도 / retry / callback 아쉽다 (파이썬)               |
|      |- 자사의 유료 제품이 기본 디펜던시로 포함되어 있음     |- 편의기능 아쉽고, 직접 구현해야 하는 부분이 많음              |
...
- 컨텍스트
  ...
  - 할루시네이션 감소
  - 포함 관련 정보들
    - 도메인 관련 지식, 노하우 같은 세부 정보 (용어도 직접적으로 설명)
    - 개인화 정보
   
- 페르소나
  - 답변의 어조 및 스타일을 형성하는데 중요한 역할
  - 입력해주는 것들
    - 현재 상황(맥락) : 너는 초등학생에게 교습을 해주고 있어
    - 직업(전문성) : 너는 어려운 내요을 쉽게 알려주는 초등학교 선생님이야
    - 어조 : 친구에게 대하듯 편하게 말해줘 
